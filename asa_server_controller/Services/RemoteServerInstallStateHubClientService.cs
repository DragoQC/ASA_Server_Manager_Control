...
using asa_server_controller.Models.Install;
using asa_server_controller.Models.Servers;
using Microsoft.AspNetCore.SignalR.Client;

namespace asa_server_controller.Services;

public sealed class RemoteServerInstallStateHubClientService(
    IServiceScopeFactory serviceScopeFactory,
    ILogger<RemoteServerInstallStateHubClientService> logger) : BackgroundService
{
    private readonly ConcurrentDictionary<int, RemoteHubRegistration> _connections = new();
    private readonly ConcurrentDictionary<int, RemoteInstallStateSnapshot> _snapshots = new();
    private readonly SemaphoreSlim _synchronizeLock = new(1, 1);
    public event Action<int>? Changed;

    public RemoteInstallStateSnapshot GetSnapshot(int remoteServerId)
    {
        return _snapshots.TryGetValue(remoteServerId, out RemoteInstallStateSnapshot? snapshot)
            ? snapshot
            : RemoteInstallStateSnapshot.Default(remoteServerId);
    }
    ...
    {
        Uri hubUri = new($"{server.BaseUrl.Trim().TrimEnd('/')}{AdminInstallStateHubConstants.Route}", UriKind.Absolute);
        ...
        _snapshots[server.Id] = RemoteInstallStateSnapshot.Default(server.Id);

        connection.On<RemoteInstallWorkspaceStatusSnapshot>(AdminInstallStateHubConstants.WorkspaceUpdatedMethod, workspace =>
        {
            _snapshots.AddOrUpdate(
                server.Id,
                _ => RemoteInstallStateSnapshot.Default(server.Id) with
                {
        ...
        connection.On<RemoteInstallProgressSnapshot>(AdminInstallStateHubConstants.ProgressUpdatedMethod, progress =>
        {
            _snapshots.AddOrUpdate(
                server.Id,
                _ => RemoteInstallStateSnapshot.Default(server.Id) with
        ...
        {
            logger.LogWarning(error, "Remote install-state hub reconnecting for server {RemoteServerId}.", server.Id);
        ...
        connection.Closed += error =>
        {
            ...
            {
                logger.LogWarning(error, "Remote install-state hub closed for server {RemoteServerId}.", server.Id);
            }
    ...
    private async Task EnsureStartedAsync(int remoteServerId, HubConnection connection, CancellationToken cancellationToken)
    {
        ...
        {
            logger.LogWarning(exception, "Unable to connect to remote install-state hub for server {RemoteServerId}.", remoteServerId);
    ...
    private void UpdateConnectionState(int remoteServerId, string connectionState)
    {
        _snapshots.AddOrUpdate(
            remoteServerId,
            _ => RemoteInstallStateSnapshot.Default(remoteServerId) with
            {
    ...
    private void NotifyChanged(int remoteServerId)
    {
        ...
        foreach (Action<int> handler in handlers.GetInvocationList().Cast<Action<int>>())
        {
            ...
            {
                logger.LogWarning(exception, "Remote install-state hub subscriber failed for server {RemoteServerId}.", remoteServerId);
            }
        }
    }
