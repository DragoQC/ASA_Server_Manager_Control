...
SMB_APPLY_SCRIPT_TEMPLATE_RELATIVE_PATH="asa_server_controller/Templates/Cluster/apply-smb-server.sh"
SMB_APPLY_SCRIPT_PATH="${SMB_DIR}/apply-smb-server.sh"
...
${USER_NAME} ALL=(root) NOPASSWD: ${SMB_APPLY_SCRIPT_PATH}
...
if [ -f "${REPO_DIR}/${SMB_APPLY_SCRIPT_TEMPLATE_RELATIVE_PATH}" ]; then
  cp "${REPO_DIR}/${SMB_APPLY_SCRIPT_TEMPLATE_RELATIVE_PATH}" "${SMB_APPLY_SCRIPT_PATH}"
  chown root:root "${SMB_APPLY_SCRIPT_PATH}"
  chmod 0755 "${SMB_APPLY_SCRIPT_PATH}"
fi
